...
def send_spam(uid):
    try:
        ...
        api_url = f"https://bngxspam-inv.onrender.com/spam?id={uid}"
        response = requests.get(api_url)
...
def send_likes(uid):
    likes_api_response = requests.get(f"https://bngx-like-bott.onrender.com/add_likes?uid={uid}&key=BNGX")
    ...
    if likes_api_response.status_code == 200:
        ...
        if api_data.get("LikesGivenByAPI", 0) == 0:
            ...
            return {
                "status": "failed",
                "message": (
                    f"[C][B][00FF00]تمت زبادة 100 لايك بنجاح"
                    f"________________________"
...
[C][B][ffffff]- اهلا بيكا في بوتك المتواضع
          [ffd319]        S1X_AMIN

[C][B][6E00FF]- To display commands type :

[ffffff]-  [ @help ]           

[C][B][ffffff]- NF FAMILY 

 the account ✓

[b][c][FF0000]-----------------------------------
[00FF00] Instagram: @7r9_bellara
[C][B][FFD3EF]-Telegram :@S1X_AMIN
[C][B][FFD3EF] YouTube :S1X_AMIN
[b][c][FF0000]-----------------------------------""",uid
                            )
                        )
                else:
            ...
            if "1200" in data.hex()[0:4] and b"/admin" in data:
                ...
                clients.send(
                    self.GenResponsMsg(
                        f"""[C][B][FF00FF] AKIRA FF 

[C][B][FF0000]هل تفكر [C][B][00FF00]في الحصول [C][B][FFFF00]على البوت [C][B][0000FF]بسعر جيد؟ [C][B][FF00FF]🔥

[C][B][00FFFF]هل ترغب [C][B][FF4500]في لوحة تحكم [C][B][7CFC00]بدون قيود؟ 🛠️

[C][B][FFA500]هل تحلم [C][B][00CED1]بسيرفر باسمك [C][B][FF1493]ولوحة احترافية؟ 🚀

[C][B][00FF00]كل هذا [C][B][FF69B4]متوفر [C][B][FFD700]فقط [C][B][FF0000]تواصل معنا الآن! 💬

[B][I][A5E2CF]💎  S1X_AMIN 💎""", uid
                    )
                )
...
سبام طلبات انضمام للفريق :
  /ُ😂sp <id> 

سبام طلبات انضمام للروم :
  /ُ😂room <id> 
...
اضافة 100 لايك للاعب :
  /ُ🤣likes <id>""",uid
                        ...
                        clients.send(
                                    self.GenResponsMsg(
                                        f"""
        [c][b]•  دعوة لاعب معك للفريق:
 /ُ😃send <id>
                          
لاغ عبر تيم كود :
 /ُ🤣lag <team cod> 

تدمير السكواد عبر تيم كود:
 /ُ🤣rest <team cod> 

سبام رسائل في الفريق
  /ُ🤣spm <team code> <message>
 
بدء  اجباري  عبر التيم كود :
  /ُ🤣start <team cod>""",uid
                            )
                        )                               
...
ارسال طلبات صداقه للاعب :
  /ُ🤣spam <id>
  
  لجعل البوت يدخل معك السكواد:
 /😅come <team code>
        
        لجعل البوت يخرج من السكواد:
 /🤣solo

 انضمام خفي للفريق (Ghost Mode):
 /🤣ghost <team code>
                ...
                try:
                    ...
                    if len(command_parts) < 2:
                        ...
                        try:
                            ...
                            ghost_message = f"[C][B][FF1493]👻 AKIRA VIP GHOST IS HERE 👻\n[C][B][00FFFF]🔥 PREMIUM BOT ACTIVATED 🔥"
                            ...
                            clients.send(
                                self.GenResponsMsg(
                                    ...
                                    f"[C][B][FF1493]👻 AKIRA VIP GHOST MODE\n"
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="4240539757", password="B250E08FD4B726DD159ED7D5EB8C700220AFB4592374F41A015F7E829D79AA64")
        client_thread.start()
    except Exception as e:
